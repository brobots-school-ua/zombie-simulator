...
# Fix Plan — Round 17: Два нові типи зомбі + баланс HP

## 48. Walker HP 30→50, Runner HP 20→35

## 49. Radioactive зомбі
- HP: 40, Speed: 50, Damage: 5, Score: 30, Coins: 2
- Detection: 300, з'являється з wave 4+
- Фішка: аура отрути ~60px — 2 HP/сек гравцю поки поруч
- При смерті: токсична калюжа 5 сек що дає 3 HP/сек
- Текстура: зелене сяйво, токсичні бульбашки

## 50. Kamikaze зомбі
- HP: 15, Speed: 180, Damage: 0 (контакт), Score: 25, Coins: 2
- Detection: 9999 (бачить через всю карту, завжди агро)
- Фішка: вибухає при контакті — 35 урону, радіус 70px
- При смерті від куль: менший вибух 40px, 15 урону
- Мигає червоним під час руху
- З'являється з wave 5+

**Files:** `src/entities/Zombie.ts`, `src/scenes/BootScene.ts`, `src/scenes/GameScene.ts`

---

# Fix Plan — Round 16: Колізії зомбі, корона, рефанд, превʼю аксесуарів

## 48. Зомбі мають колізію з гравцем
- Замінити overlap на collider для zombie-player
- Зомбі впираються в гравця а не проходять наскрізь
- Damage через окремий overlap з перевіркою дистанції
**Files:** `src/scenes/GameScene.ts`

## 49. Корона вище на гравці
- Підняти offsetY для корони (-14 замість -10)
**Files:** `src/systems/ShopConfig.ts`

## 50. Рефанд аксесуарів
- Кнопка "Refund" біля куплених аксесуарів
- Повертає повну ціну в монети
- Прибирає аксесуар з owned, знімає якщо equipped
**Files:** `src/systems/ShopConfig.ts`, `src/scenes/MenuScene.ts`

## 51. Превʼю аксесуарів перед покупкою
- В магазині показувати спрайт гравця з обраним аксесуаром
- Клік на аксесуар = превʼю на моделі
- Як в GTA — бачиш як виглядає перед покупкою
**Files:** `src/scenes/MenuScene.ts`
