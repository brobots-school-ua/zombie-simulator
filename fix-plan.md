...
# Fix Plan — Round 18: Адмін-спавн зомбі

## 51. Система спавну зомбі через адмін-панель

### Крок 1: UI в адмін-панелі
- Нова секція "Spawn Zombie" в AdminConsole
- Квадрат-превʼю обраного зомбі (за замовчуванням — walker)
- Клік на превʼю → відкриває повноекранне вікно вибору
- Вікно вибору: всі 5 типів зомбі (walker, runner, tank, radioactive, kamikaze) — великі квадрати з зображенням
- Клік на тип → вибирає його, закриває вікно
- Поле "Count" (кількість, 1-10)
- Кнопка "Spawn" — запускає спавн

### Крок 2: Спавн процес
- При натисканні Spawn:
  1. Закривається адмін-панель
  2. На позиції гравця з'являється червоний кружечок-маркер
  3. Над маркером — таймер "5...4...3...2...1" + зображення зомбі
  4. Через 5 секунд: маркер зникає, на його місці спавниться зомбі
  5. Якщо count > 1: додаткові зомбі спавняться в рандомних точках навколо першого (радіус 50-100px), з тим же таймером

### Крок 3: Технічна реалізація
- `AdminConsole.ts` — додати UI вибору зомбі + count + spawn кнопку
- `GameScene.ts` — додати публічний метод `adminSpawnZombie(type, x, y, delay)`
  - Створює маркер (червоний кружечок)
  - Показує таймер + текстуру зомбі
  - Через delay секунд — спавнить реального зомбі

### Files:
- `src/systems/AdminConsole.ts` — UI
- `src/scenes/GameScene.ts` — метод adminSpawnZombie
